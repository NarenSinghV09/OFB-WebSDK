(function($){
	...
	customTemplate.prototype.renderMessage = function (msgData) {
		...
		else if (msgData && msgData.message && msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "agent_start_typing") {
			$(".typingIndicatorContent").addClass("agentIsTyping");
			setTimeout(function() {
				$(".typingIndicatorContent").removeClass("agentIsTyping");
			}, 3000);
            if(!(msgData.fromHistory)) {
				messageHtml = "<div></div>";
            }else{
                return messageHtml = "_ignore_message_render_";
            } 
        }
